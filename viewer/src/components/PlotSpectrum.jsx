...
const PlotSpectrum = (props) => {
    const propsLoading = props.loading ? props.loading : false
    const propsHeight = props.height ? props.height : null
    const propsWidth = props.width ? props.width : null
    const propsSpecA = props.specA ? props.specA : {}
    const propsSpecB = props.specB ? props.specB : {}
    ...
    useEffect(() => {
        const data = {plotType: stateDisplayMode, specA: propsSpecA, specB: propsSpecB}
        if (data.specA.peaks || data.specB.peaks) {
            const dataPlot = funcPlotSpectrum(data)
            if (propsHeight && propsWidth) {
                dataPlot.layout.height = propsHeight
    ...
    }, [propsSpecA, propsSpecB, stateDisplayMode, propsHeight, propsWidth])
            ...
            <Row justify={"center"} align={"middle"}>
                <Col span={24} style={{height: propsHeight * 1.2, width: propsWidth * 1.2}}>
                    {(stateLoading || (!((stateData.data && stateData.data.length > 0)))) ? <Row justify={"center"}>
                        ...
                        <Plot
                            style={{
                                position: 'relative', display: 'inline-block', overflow: 'hidden', margin: '0 auto',
